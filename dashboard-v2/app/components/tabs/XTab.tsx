...
export default function XTab() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/sns-metrics?platform=x&days=30').then(r => r.json()),
      fetch('/api/post-metrics?platform=x&limit=100').then(r => r.json()),
    ]).then(([s, p]) => {
      setSns(s.metrics ?? []);
      setPm(p.metrics ?? []);
      setLoading(false);
  ...
  // weekly ER from sns_metrics promoAvgER / normalAvgER
  ...
  return (
    <>
      <KpiGrid items={[
        ...
        [erBarData.length ? `${(erBarData[erBarData.length - 1] as Record<string, unknown>)?.['プロモ'] ?? '—'}` : '—', '最新プロモER'],
      ]} />
        ...
        {topPosts.length ? (
          <div style={{ maxHeight: 260, overflowY: 'auto' }}>
            <table className="w-full border-collapse">
              <thead><tr><TH>post_id</TH><TH>アカウント</TH><TH>インプレッション</TH><TH>記録日</TH></tr></thead>
