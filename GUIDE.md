...
"hooks": {
  "PreToolUse": [
    ...
    },
    {
      ...
      "hooks": [{ "type": "command", "command": "python3 /Users/Rascal/.claude/plugins/marketplaces/claude-plugins-official/plugins/security-guidance/hooks/security_reminder_hook.py", "timeout": 8 }]
    },
    {
      "matcher": "mcp__*",
...
> 別PCに再現する場合は `/Users/Rascal/` を実際のホームディレクトリに置き換えてください。
...
python3 -c "import json; d=json.load(open('/Users/Rascal/.claude/settings.json')); print('plugins:', d.get('plugins', []))"

# ⑩ Hooks 確認
python3 -c "import json; d=json.load(open('/Users/Rascal/.claude/settings.json')); print('hook types:', list(d.get('hooks',{}).keys()))"
