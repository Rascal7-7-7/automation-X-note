...
→ テンプレートに合わせて最も強い1つを選ぶ

【AIツール名 + 再現性ヒント（重要）】
- script[2]: ツール名 + 再現性の示唆（最大14文字）
- 形式: 「Runwayで作れます」「スマホでも作れる」「誰でも無料で作れる」
- 再現性を匂わせることでフォロー率が跳ねる

【絶対ルール】
- JSONのみ出力（説明・マークダウン不要）
- hookText: 12文字以内・上記3パターンのどれか
- script: 7要素の配列（絵文字・記号禁止）
  ...
  - script[5]: 最大16文字（崩壊 or ループ。必須。「最初に戻る」「世界が壊れる」）
