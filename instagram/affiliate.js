...
const REELS_SYSTEM = `以下のアフィリエイト商品向けにInstagram Reels用の短い台本（15〜30秒）を作成してください。
構成: 冒頭3秒フック → 商品紹介（具体的なメリット） → CTA（リンクから詳細を見てください）
Renoise等の動画生成AIに渡すことを想定し、自然な話し言葉で書いてください。
アフィリエイトURLを直接読み上げず「プロフィールのリンクから」と誘導してください。`;
...
export async function runAffiliateGenerate({ today, draftDir, forceCampaignId } = {}) {
  const campaigns = loadCampaigns();
  const campaign  = forceCampaignId
    ? (campaigns.find(c => c.id === forceCampaignId) ?? selectCampaign(campaigns))
    : selectCampaign(campaigns);
