...
// バズる動画の型（@develogon0 の戦略 + @gagarot200 の7フック戦略を統合）
const BUZZ_TYPES = [
  ...
  // @gagarot200 の7フック戦略（AI副業・自動化テーマに適応）
  { id: 'I', name: '痛み直撃型',       hook: '副業したいけど時間も知識もない人へ' },
  { id: 'J', name: '常識破壊型',       hook: 'AIで副業は稼げないは全部勘違いです' },
  { id: 'K', name: '数字証拠型',       hook: '月◯万円の根拠を全部公開します' },
  { id: 'L', name: '秘密暴露型',       hook: '誰も教えてくれないAI活用の裏技' },
  { id: 'M', name: '質問巻き込み型',   hook: 'あなたはこのAIツール使えてますか？' },
  { id: 'N', name: '劇的変化型',       hook: '3ヶ月前の自分に伝えたかったこと' },
  { id: 'O', name: '感情物語型',       hook: '諦めかけていた副業がAIで変わった話' },
  { id: 'P', name: 'AI透明性型',      hook: 'これAIが書きました（でも本当に役立つ情報）' },
  { id: 'Q', name: '文化的交差点型',  hook: 'AI×人間の対比で見えてくること' },
];
...
const ACCOUNT_PROFILES = {
  1: {
    ...
    cta:     'プロフのリンクから👆',
    // #副業 はInstagramで制限タグのため除外。代替タグを使用。
    hashtags: '#AI活用 #ChatGPT #生成AI #スキルアップ #フリーランス',
  },
  2: {
    name:    'アフィリ集客系',
    ...
    cta:     'おすすめ案件一覧はプロフから🔗',
    // #副業 はInstagramで制限タグのため除外
    hashtags: '#アフィリエイト #集客 #SNSマーケティング #インスタ集客 #スキルアップ',
  },
};
...
指定された「バズる型」に従ってInstagramキャプションを作成してください。

【構造（この順番で書く）】
1行目（フック）: 28文字以内・「続きを読む」前に全表示される唯一の行。型に合ったフックを書く。絵文字は末尾1個まで。
★必ず数字を含める（月○万・○時間・○ステップ・○%・○日間 等）。数字なしフックは禁止。
★例: 「月3万増えた1つの習慣」「3ステップで始めるAI副業」「30分で稼ぐ仕組みを作った話」
（空行）
本文: 有益な情報を3〜5点。各項目は「・」か絵文字で始め、1項目1行で改行。
★最低1点は「月○万・○時間・○%削減」等の具体的数字を含む実績・目安を書く。
★「うまくいく」「効果的」等の抽象表現だけはNG。必ず根拠・数字・具体例を添える。
（空行）
保存CTA: 「後で読み返せるように保存👆」「完全版は保存が便利」のどちらか。
（空行）
DMシェアCTA: 「これ、〇〇で悩んでる友人に送ってあげてください📩」「この情報を必要としてる人のDMに送ってあげてください」のどちらか。（DM送信がアルゴリズム最高評価指標。「シェア」より「送る」「DM」を明示する）
（2行空け）
ハッシュタグ: 3〜5個のみ（関連度が高いもの厳選。必ず含める: ${p.hashtags}）
...
【品質指定（必ず含める）】
"designed by a world-class award-winning graphic designer, trending design, commercial-grade visual quality"

【ビジュアル指定】
- クリーンなデザイン、インフォグラフィック風、白または薄いグレー背景、日本語テキストなし（後でオーバーレイ）
- 型に合わせたビジュアル:
  - ランキング・比較 → 4分割グリッドレイアウト（各セクション異なるトーン）
  - How-to・手順 → 縦型ステップ図（番号付き）
  - 実績・数字 → ダッシュボード風インフォグラフィック
  - Before/After → 左右2分割コントラストデザイン
  - ツール紹介 → アイコン+テキストカード型グリッド
- フィード用は横幅を活かした横展開レイアウト
- Reels/Story用は縦長を活かした縦スクロール視線誘導
...
export async function runGenerate({ account = 1, type } = {}) {
  ...
  logger.info(MODULE, `caption review score: ${captionReview?.avgScore ?? 'n/a'}`);
