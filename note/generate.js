...
【tagsのルール】
- 5個のハッシュタグ（#なし）
- AI副業・Claude・n8n・自動化・副収入など記事テーマに直結するキーワード
...
async function generateOutline(theme, hintText, account) {
  ...
  const raw = await generate(
    OUTLINE_SYSTEM,
    `テーマ: ${theme}${hintText}${extra}${personaExtra}`,
...
- Markdown形式（## 見出し、**強調**、\`\`\`コードブロック\`\`\`、| テーブル |）
...
- 【証拠セクション必須】各セクションに1つ以上の証拠を入れる:
  ...
  - スクリーンショット言及: 「※以下は実際の[ツール名]画面」と記載してプレースホルダー画像を示す
